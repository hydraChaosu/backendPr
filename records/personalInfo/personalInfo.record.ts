...
export class PersonalInfoRecord implements PersonalInfoEntity {
  ...
  async insert(): Promise<string> {
    ...
    await pool.execute(
      "INSERT INTO `personalinfo`(`id`, `userId`, `name`, `surname`, `city`, `country`, `street`, `buildingNumber`,`postalCode`) VALUES(:id, :userId, :name, :surname, :city, :country, :street, :buildingNumber, :postalCode)",
      {
        id: this.id,
        userId: this.userId,
        name: this.name,
  ...
  async update(): Promise<void> {
    await pool.execute(
      "UPDATE `personalinfo` SET `name` = :name, `surname` = :surname, `city` = :city, `country` = :country, `street` = :street, `buildingNumber` = :buildingNumber, `postalCode` = :postalCode WHERE `id` = :id",
      {
        id: this.id,
        name: this.name,
        surname: this.surname,
  ...
  static async getUserInfo(userId: string): Promise<PersonalInfoRecord> {
    const [results] = (await pool.execute(
      "SELECT * FROM `personalinfo` WHERE `userId` = :userId",
      {
        userId,
      }
