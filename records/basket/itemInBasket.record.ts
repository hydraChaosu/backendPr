...
export class ItemInBasketRecord implements ItemInBasketEntity {
  ...
  async insert(): Promise<string> {
    ...
    await pool.execute(
      "INSERT INTO `basketelements`(`id`, `shopItemId`, `userId`, `quantity`) VALUES(:id, :shopItemId, :userId, :quantity)",
      {
        id: this.id,
        userId: this.userId,
  ...
  async update(): Promise<void> {
    await pool.execute(
      "UPDATE `basketelements` SET `quantity` = :quantity WHERE `id` = :id",
      {
        id: this.id,
        quantity: this.quantity,
      }
  ...
  async updateFull(): Promise<void> {
    await pool.execute(
      "UPDATE `basketelements` SET `quantity` = :quantity, `shopItemId` = :shopItemId, `userId` = :userId WHERE `id` = :id",
      {
        id: this.id,
        quantity: this.quantity,
        shopItemId: this.shopItemId,
  ...
  static async listAllItemsForUser(
    userId: string
  ): Promise<ItemInBasketRecord[]> {
    const [results] = (await pool.execute(
      "SELECT * FROM `basketelements` WHERE  `userId` = :userId",
      {
        userId,
      }
  ...
  static async listAllSameShopItems(
    itemId: string
  ): Promise<ItemInBasketRecord[]> {
    const [results] = (await pool.execute(
      "SELECT * FROM `basketelements` WHERE  `shopItemId` = :itemId",
      {
        itemId,
      }
    )) as ItemInBasketRecordResults;
  ...
  static async getOne(id: string): Promise<ItemInBasketRecord | null> {
    const [results] = (await pool.execute(
      "SELECT * FROM `basketelements` WHERE `id` = :id",
      {
        id,
      }
    )) as ItemInBasketRecordResults;
  ...
  static async deleteAllItemsForUser(userId: string): Promise<void> {
    await pool.execute(
      "DELETE FROM `basketelements` WHERE  `userId` = :userId",
      {
        userId,
      }
    );
